...
const SearchBar = () => {
	...
	const fetchSongs = async (searchTerm: string) => {
		...
		setLoading(true);
		try {
			const response = await axios.get(`http://localhost:8000/api/songs/search?query=${searchTerm}`);
			...
			{query && (
				<div className="absolute left-0 mt-2 w-64 bg-zinc-900 border border-zinc-700 rounded-md shadow-lg max-h-60 overflow-y-auto">
					{loading ? (
						...
						results.map((song) => (
							<div key={song._id} className="px-4 py-2 hover:bg-zinc-800 cursor-pointer">
								<p className="text-white text-sm font-medium">{song.title}</p>
								<p className="text-xs text-zinc-400">{song.artist}</p>
							</div>
						))
					) : (
						<div className="px-4 py-2 text-sm text-zinc-400">No songs found</div>
					)}
				</div>
			)}
		</div>
